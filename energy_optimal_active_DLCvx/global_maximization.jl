...
function maximization_cost()
    ...
    uniform_time_sample = range(0, stop=(terminal_time - 1e-4), length=num_params) |> collect
    time_sample = uniform_time_sample
    initial_state = [400, 400, 500, -10, -10, -105]
    cost = -internal_minimization(num_samples, num_params, time_sample, terminal_time, initial_state)
    tracker.best = cost
    push!(best_vals, cost)
    println("Current maximization cost: ", cost)
    return cost
end
...
uniform_time_sample = range(0.0, stop=terminal_time, length=num_params) |> collect
# @benchmark maximization_cost() samples=10 evals=1 seconds=60
...
control_trajectory(num_samples, num_params, uniform_time_sample, terminal_time, initial_state)
###############################################
